...
class CircuitGraph : public Graphm {
    ...
    vector<vector<Complex>> admitMatrix()
    {
        ...
        for (int i = 0; i < n(); ++i)
        {
            ...
            for (int j = 0; j < n(); ++j)
            {
                if (adjMatrix()[i][j] == Complex {0, 0}) {continue;}
                sum += Complex (1) / adjMatrix()[i][j];
            }
            admit_matrix[i][i] = sum;
            for (int j = 0; j < n(); ++j) // off-diagonal term
            {
                if (i != j && adjMatrix()[i][j] != Complex {0, 0})
                {
                    admit_matrix[i][j] = Complex (-1) / adjMatrix()[i][j];
                }
            }
        }
        return admit_matrix;
