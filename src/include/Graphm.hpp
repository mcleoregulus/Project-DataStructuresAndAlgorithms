...
private:
    ...
    vector<vector<Complex>> matrix;       // Use vector of vectors for adjacency matrix
...
public:
    ...
    void Init(int n) // Initialize the graph
    {
        ...
        matrix.resize(numVertex, vector<Complex>(numVertex, Complex(0, 0)));
    }
    ...
    { // Delete edge (v1, v2)
        if (matrix[v1][v2] != Complex(0, 0))
            numEdge--;
        matrix[v1][v2] = Complex(0, 0);
    }
    bool isEdge(int i, int j) // Is (i, j) an edge?
    {
        return matrix[i][j] != Complex(0, 0);
    }
    Complex weight(int v1, int v2) { return matrix[v1][v2]; } // changed type in ADT
    int getMark(int v) { return mark[v]; }
    void setMark(int v, int val) { mark[v] = val; }

    vector<vector<Complex>> getAdjMatrix() const { return matrix; }

    void addVertex() {      // 新增节点
        numVertex++;
        matrix.resize(numVertex);
        for (auto& row : matrix) {
            row.resize(numVertex, Complex(0, 0)); // 调整列数
        }
    }
